...
/**
 * The stage is already named on the axis, so a colour per bar would only be
 * decoration. Bars carry the accent; the two stages that mean "done" carry the
 * pass colour, because that is a state worth reporting.
 */
const stageFill = (stage: string) =>
  stage === 'dispatched' || stage === 'delivered' ? token('--ok') : token('--accent')
          ...
          {stageData.length === 0 ? <Empty>No open orders.</Empty> : (
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={stageData} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="stage" tick={{ fontSize: 11, fill: token('--text-muted') }} interval={0} angle={-20} textAnchor="end" height={54} />
                <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: token('--text-muted') }} />
                <Tooltip cursor={{ fill: token('--surface-sunk') }} />
                <Bar dataKey="count" radius={[6, 6, 0, 0]}>
                  {stageData.map((d) => <Cell key={d.key} fill={stageFill(d.key)} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          )}
          ...
          {attByDay.length === 0 ? <Empty>No attendance marked yet.</Empty> : (
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={attByDay} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke={token('--line')} vertical={false} />
                <XAxis dataKey="date" tick={{ fontSize: 11, fill: token('--text-muted') }} />
                <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: token('--text-muted') }} />
                <Tooltip />
                <Line type="monotone" dataKey="present" stroke={token('--accent')} strokeWidth={2} dot={{ r: 2 }} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </Card>
